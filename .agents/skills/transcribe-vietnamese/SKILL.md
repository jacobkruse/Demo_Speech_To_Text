...
- Luôn dùng `--output` rồi đọc file kết quả — đáng tin cậy hơn parse stdout, nhất là audio dài.
...
- Nếu venv chưa tồn tại (máy mới clone repo): làm theo mục "Cài đặt" trong README.md trước,
  chú ý phần lưu ý GPU trong AGENTS.md.
...
```powershell
& ".venv\Scripts\python.exe" "dictate.py" --output "ghichu.txt"
```

Dừng ghi: người dùng **ngừng nói ~2s** (tự dừng) hoặc **bấm Ctrl+C** để dừng ngay (vẫn phiên
âm phần đã ghi). KHÔNG dùng phím gõ (ENTER...) để dừng vì khi tiến trình không có stdin tương
tác sẽ ném EOFError; Ctrl+C dùng được vì là tín hiệu. Sau khi dừng, đọc `ghichu.txt` rồi xử lý
tiếp theo yêu cầu của người dùng. Biến thể:

- `--silence 3` — nói chậm/ngắt nhiều thì nới thời lặng để khỏi bị cắt giữa câu (mặc định 2.0s).
- `--seconds 15` — ghi cố định 15 giây.
