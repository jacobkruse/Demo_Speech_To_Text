...
nói (do nói quá nhỏ, nói trễ, hoặc **sai micro mặc định** — vd đang đeo tai nghe nhưng Windows
vẫn để mic máy). Script cố ý KHÔNG phiên âm trong trường hợp này để tránh Whisper "ảo giác" ra
câu rác (vd "Hãy subscribe cho kênh..."). Cách xử lý: liệt kê mic bằng
`!.venv/Scripts/python dictate.py --list-devices`, rồi chỉ định đúng mic bằng
`--device <chỉ_số hoặc tên>` (vd `--device airpods`). Hoặc khuyên người dùng nói to/rõ và bắt
đầu nói ngay sau dòng "Đang ghi âm".
